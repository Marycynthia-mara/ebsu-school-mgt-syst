...
foreach ($no_of_studs as $no_of_stud) {
	...
	if ($comment) {
		if($comment['teachers_comment'] !== NULL or $comment['teachers_comment'] !== ""){
			$tea_comt = true;
		}else{
			$tea_comt = false;
		}
...
<body>
<div class="container">
<form method="post" action="<?php echo "stud_result_sheet.php?stud_id=$stud_id" ?>">
		...
		<table class="attendance">
			<caption>ATTENDANCE RECORD</caption>
				...
				<tr>
					...
					<td class="numberInput"><input type="number" name="student_attd_pre"  value="<?php if($tea_comt){
						echo $comment['student_attd_pre'];
					} ?>"></td>
				</tr>
				<tr>
					<td class="justified">No. of times Absent</td>
					<td class="numberInput"><input type="number" name="student_attd_abs"  value="<?php if($tea_comt){
...
<div class="resultSum">
	...
	<table>
		<tr>
			...
			<td><textarea  name="teachers_comment"><?php if($tea_comt){
						echo $comment['teachers_comment'];
					} ?></textarea></td>
			<td style="border:none; width: 4em; vertical-align: bottom;">Date : </td>
			<td style="border:none; border-bottom: 1px solid #000; width: 8em;"><input type="date"  name="teachers_comt_date" value="<?php if($tea_comt){
...
<?php if($tea_comt === true){?>
	<button class="back"><a class="" href="#content1">Update Comment</a></button>
<?php }else{ ?>
...
<div class="special_action_confirm"><div id="content1" class="popup-effect"><div class="popup"><div class="letter-w3ls"><form method="post"><h1 class="ebsu">EBSU STAFF SECONDARY SCHOOL ABAKALIKI.</h1><section><p>Are you sure you want to <?php if($tea_comt === true){echo 'Update Comment';}else{echo 'Upload Comment';}?>?</p></section><div class="btnn"><button type="submit" name="submit">Proceed</button><button class="btn btn-primary bd-0">Cancel</button><br></div></form></div></div></div></div>

<?php
 if (isset($_POST['submit'])) {
  extract($_POST);
    $result = val_fetch_form3($_POST);
...
<?php if ( isset($msg
  ...
  <script>
	...
	setTimeout(function refresh(){
		window.location = 'stud_result_sheet.php?stud_id=<?php echo $stud_id?>';
	}, 1000);
</script> 
<?php endif; ?>
