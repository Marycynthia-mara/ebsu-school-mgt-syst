...
if (!isset($_GET['admin_id'])) {
  ...
  $user = fetch_user('administrators', 'admin_id', $admin_id);
  if($user){
    extract($user);
  }else{
    redirect_to('dashboard.php');
...
if (isset($_POST['submit'])) {
    $result = update_admin($_POST, $admin_id);
...
<?php if ( isset($msg
    ...
    setTimeout(function refresh(){
		window.location = 'edit-admin.php?admin_id=<?php echo $admin_id?>';
	}, 1000);
</script> 
<?php endif; ?>
  ...
  <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']).'?admin_id='."$admin_id"; ?>">
   <div class="section-wrapper mg-t-20">
          <label class="section-title">Admin Update Page</label>
