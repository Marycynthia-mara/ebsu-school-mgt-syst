...
if($_SESSION['user_id'] !== $_GET['stud_id']){
	redirect_to('dashboard.php');
}
...
<?php
if (isset($_GET['stud_id'])) {
	$current_student = $_GET['stud_id'];
	$pin_serial = fetch_column4('student_id', 'students', $current_student);
	if($pin_serial){
		extract($pin_serial);
	}else{
		redirect_to('dashboard.php');
	}
    $result = fetch_column11('all_pin_code', 'student_id', 'students', $current_student, $pin, $serial_no);
    if ($result === true) {
...
?>
      <?php if ($errors) { ?>
        <script>
            ...
            var allAlerts = '<p style="color:#F27474;text-align:center;"><b>' + '<h4 style="text-align:center;color:#F27474;">' + 'UNAUTHORIZED ACCESS.' + '</h4>'  + '<br>' + 'Read the below stated issue(s).' + '</b></p>';
            var i;
            var timer = 0;
            for(i in sweetAlert){
...
<body>
<div class="container">
<form method="post" action="<?php echo "stud_result_annual_sheet_p_comm.php?stud_id=$stud_id" ?>">
  ...
  <header>
    ...
    <table>
      <tr>
        <td>Student Name : </td>
        <td><input type="text" readonly="" name="" value="<?php echo ucfirst($surname) .' '. ucfirst($firstname) .' '. ucfirst($lastname);?>"></td> 
        <td>Class : </td>
    ...
    <table class="attendance">
      <caption>ATTENDANCE RECORD</caption>
        ...
        <tr>
          ...
          <td class="numberInput"><input type="text" readonly name="student_attd_pre" <?php if($tea_comt){
            // echo "readonly";
        ...
        <tr>
          ...
          <td class="numberInput"><input type="text" readonly name="student_attd_abs" <?php if($tea_comt){
            // echo "readonly";
          } ?> value="<?php if($tea_comt){
...
<div class="resultSum">
  <table>
    ...
    <tr >
       <td>Decision: </td>
      <td><input style="margin-top: 20px;" readonly="" type="text" <?php if($tea_comt){
        // echo "readonly";
  ...
  <table>
    <tr>
      ...
      <td><textarea readonly style="width: 100%;" <?php if($tea_comt){
        // echo "readonly";
      ...
      <td style="border:none; border-bottom: 1px solid #000; width: 8em;"><input readonly type="date" <?php if($tea_comt){
        // echo "readonly";
    ...
    <table> 
      <tr> 
        ...
        <td style="border:none; border-bottom: 1px solid #000; width: 20em;"><input type="text" readonly value="<?php if($tea_comt){
            echo $comment['principals_comment'];
          } ?>" name="p_comm" <?php if($_SESSION['category'] == "staff"){
        echo "readonly";
        ...
        <td style="border:none; border-bottom: 1px solid #000; width: 5em;"><input readonly <?php if($_SESSION['category'] == "staff"){
        echo "readonly";
